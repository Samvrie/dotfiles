...
echo ''
read -p "Do you want to use samvrie's dotfiles? y/n" -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    echo ''
	echo "Now pulling down samvrie dotfiles..."
    ...
    if [[ $? -eq 0 ]]
    then
        echo "Successfully configured your environment with samvrie's macOS dotfiles..."
    else
        echo "samvrie's macOS dotfiles were not applied successfully..." >&2
fi
else 
	echo ''
    echo "You chose not to apply samvrie's macOS dotfiles. You will need to configure your environment manually..."
	echo ''
